...
class RevisedCampaignTests(unittest.TestCase):
    def test_revised_commands(self):
        for n, name, count in [('1M', 'b3-k5-revised', 1), ('100k', 'b3-k100-revised', 2), ('500k', 'b3-k100-revised', 2), ('1M', 'b3-k100-revised', 2)]:
            ...
            for row in rows:
                ...
                self.assertEqual(p['plaintext_modulus'], 1179649 if n == '1M' else 786433)
                refresh = '--refresh-mode=collective' in cmd
                self.assertEqual(p['logN'], 14 if refresh else 15)
                self.assertEqual(len(p['Q']), 6 if refresh else (9 if n == '1M' else 8))
