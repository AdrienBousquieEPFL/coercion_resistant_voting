...
def main():
    ...
    for file in args.screens:
        with file.open() as f:
            for row in csv.DictReader(f):screens.setdefault(row['family'],[]).append(row)
    fields=['experiment_id','parameter_file','logN','logQ_bits','logP_bits','plaintext_modulus','Q_primes','P_primes','lattice_security_bits','voters_per_ciphertext','ciphertexts','refresh_boundaries','refresh_ciphertexts','ingestion_periods_measured','input_validity_gates_measured_max','period_grid_ciphertexts','period_grid_coefficient_gib','synthetic_trials','synthetic_min_margin_bits','synthetic_status']
