        ...
        with (directory/filename).open() as f:
            for row in csv.DictReader(f):
                ...
                for metric in ['wall_ms','cpu_ms']:
                    if metric in row:
                        values[key+(f'{label}:{name}:{metric}',)].append(float(row[metric]))
    if ingestion_wall is not None:
        scope='one_input_period' if run['execution_mode']=='benchmark' else 'five_input_periods'
        values[key+(f'server_observed_wall_ms:{scope}',)].append(ingestion_wall+downstream_wall)
    if projected_ingestion_wall is not None:
        values[key+('server_projected_wall_ms:five_input_periods',)].append(projected_ingestion_wall+downstream_wall)
